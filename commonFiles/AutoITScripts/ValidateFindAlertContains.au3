...
If $alertExpected = $TRUE_STRING Then
   If $alertAppeared = True Then
	  ...
	  If StringInStr($alertMessage, $OK_BUTTON_TEXT) Then
		 $alertMessage = StringReplace($alertMessage, $OK_BUTTON_TEXT, $EMPTY_STRING, $STR_CASESENSE)
	  EndIf
	  If StringInStr($alertMessage, $CANCEL_BUTTON_TEXT) Then
		 $alertMessage = StringReplace($alertMessage, $CANCEL_BUTTON_TEXT, $EMPTY_STRING, $STR_CASESENSE)
	  EndIf
	  $alertMessage = StringStripWS($alertMessage, $STR_STRIPLEADING + $STR_STRIPTRAILING)
	  $alertMessage = StringReplace($alertMessage,@cr,"");
	  $alertExpectedMessage = StringStripWS($alertExpectedMessage, $STR_STRIPLEADING + $STR_STRIPTRAILING)
	  $alertExpectedMessage = StringReplace($alertExpectedMessage,@cr,"");
	  $alertIsValid = StringInStr($alertMessage, $alertExpectedMessage) > 0
