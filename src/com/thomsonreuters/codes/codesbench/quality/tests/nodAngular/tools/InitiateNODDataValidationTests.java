...
public class InitiateNODDataValidationTests extends TestService
{
    /**
     * HALCYONST-10602
     ...
     * and then click "Initiate NOD Data Validation" from the Tools section
     * 3. VERIFY: Initiate NOD Data Validation window opened
     * 4. VERIFY the following elemnts are present:
     * a) "Looks for issues in NOD data." heading
     ...
     * j) "Cancel" button is enabled
     * 5. Click "Cancel"
     * 6. VERIFY: Initiate NOD Data Validation window closed
     */
    @Test
    @EDGE
    @LEGAL
    @LOG
    public void initiateNODDataValidationLayoutTest()
    {
        String expectedStatus = "Looks for issues in NOD data.";
        ...
        //and then click "Initiate NOD Data Validation" from the Tools section
        homePageAngular().clickInitiateNODDataValidation();
        // VERIFY: Initiate NOD Data Validation window opened
        Assertions.assertTrue(initiateNODDataValidationPopupAngular().doesElementExist(InitiateNODDataValidationPopupElementsAngular.PAGE_TITLE), "Initiate NOD Data Validation window DID NOT open");
        //4. VERIFY the following elements are present:
        String actualStatus = initiateNODDataValidationPopupAngular().getStatus();
        String actualCurrentUserName = initiateNODDataValidationPopupAngular().getCurrentUserName();
        boolean isFileNameDisplayed = initiateNODDataValidationPopupAngular().isFileNameDisplayed();
        ...
        Assertions.assertAll(
                ...
                () -> Assertions.assertFalse(homePageAngular().doesElementExist(InitiateNODDataValidationPopupElementsAngular.PAGE_TITLE), "Initiate NOD Data Validation window DID NOT close")
        );
    }
     ...
     * HALCYONST-10602
     * 1. Log onto the Iowa (development) content set
     * 2. Go to NOD -> Subscribed Cases - angular, then click "Home" in the header on the top of the page, and then click "Initiate NOD Data Validation" from the Tools section
     * 3. VERIFY: Initiate NOD Data Validation window opened
    ...
    @LEGAL
    @LOG
    public void initiateNODDataValidationFilterSelectAllTest()
    {
        ...
        homePageAngular().openNodHomePage(ContentSets.IOWA_DEVELOPMENT);
        loginPage().logIn();
        //2. click "Initiate NOD Data Validation" from the Tools section
        homePageAngular().clickInitiateNODDataValidation();
        //3. VERIFY: Initiate NOD Update window opened
        Assertions.assertTrue(initiateNODDataValidationPopupAngular().doesElementExist(InitiateNODDataValidationPopupElementsAngular.PAGE_TITLE), "Initiate NOD Data Validation window DID NOT open");
        //4. Click in "File name" field and type there something like TEST_001
        initiateNODDataValidationPopupAngular().setFileName(fileName);
        //5. In filter field above the non-selected volumes column type "constitution"
     ...
     * 8. VERIFY: Workflow is created and hyperlink is provided
     */
    @Test
    @EDGE
    @LEGAL
    @LOG
    public void initiateNODDataValidationWorkflowTest() // Test failed due to workflow in old and new NOD can't work on the same time and now it is turned of on NODAngular
    {
        String fileName = "TEST_001";
        String firstVolume = "0030";
        String secondVolume = "0050";
        ...
        String actualMessage = initiateNODDataValidationPopupAngular().getMessage();
    ...
    @LEGAL
    @LOG
    public void initiateNODDataValidationWorkflowWithEmptyFieldTest() // Test failed due to workflow in old and new NOD can't work on the same time and now it is turned of on NODAngular
    {
        String firstVolume = "0030";
        String secondVolume = "0050";
        String thirdVolume = "0040";
