...
public class SourceNavigateAngularAssertions extends SourceNavigateAngularBase {
        ...
        for (int i = 0; i < row; i++)
            for (int j = 0; j < column; j++) {
                assertThat(!sourceNavigateAngularPage()
                        .isElementDisplayed(format(elementPath, i, selectedRow.get(j))))
    ...
    public void assertThatViewValidationColumns(List<String> columnExpectedFilterValues) {
        for (int i = 0; i < columnExpectedFilterValues.size(); i++) {
            ...
            assertThat(sourceNavigateAngularPage()
                    .isElementDisplayed(format(SourceNavigateAngularPageElements.COLUMN_NAMES, columnExpectedFilterValues.get(i))))
    ...
    public void assertThatTaxTypeAddWindowHasAllOptions(int totalAvailableTaxType) {
        ...
        assertThat(sourceNavigateAngularPage().isElementDisplayed(format(SourceNavigateAngularPageElements.FILTER_LIST, "1")))
                .as("Filter list on left panel is not displayed")
                .isTrue();
        assertThat(sourceNavigateAngularPage().isElementDisplayed(format(SourceNavigateAngularPageElements.FILTER_LIST, "2")))
    ...
    public void assertValidationsAfterAddingTaxTypes(List<String> taxTypes) {
        ...
        for (String verify : taxTypes) {
            assertThat(sourceNavigateAngularPage().isElementDisplayed(format(SourceNavigateAngularPageElements.TAX_TYPE_COULMN_ID + "[contains(text(),'%s')]", verify)))
    ...
    public void assertTheContextMenuForPrepDocUnderAllTabs(List<String> contextMenuOptions) {
        for (String verify : contextMenuOptions) {
            assertThat(sourceNavigateAngularPage().isElementDisplayed(format(SourceNavigateAngularContextMenuItemsPageElements.CONTEXT_MENU_OPTION, verify)))
    ...
    public void assertThatLockedRenditionContextMenuItems() {
        boolean flag1 = sourceNavigateAngularLockReportPage().doesElementExist(format(SourceNavigateAngularLockReportPageElements.CONTEXT_MENU_OPTION, "Unlock"));
        boolean flag2 = sourceNavigateAngularLockReportPage().doesElementExist(format(SourceNavigateAngularLockReportPageElements.CONTEXT_MENU_OPTION, "Force Unlock"));
        boolean flag3 = sourceNavigateAngularLockReportPage().doesElementExist(format(SourceNavigateAngularLockReportPageElements.CONTEXT_MENU_OPTION, "Transfer lock"));
    ...
    public void assertThatLockedRendtionContextMenuState() {
        boolean flag1 = sourceNavigateAngularLockReportPage().getElementsAttribute(format(SourceNavigateAngularLockReportPageElements.CONTEXT_MENU_OPTION_PARENT, "Unlock"), "class").contains("disabled");
        boolean flag2 = sourceNavigateAngularLockReportPage().getElementsAttribute(format(SourceNavigateAngularLockReportPageElements.CONTEXT_MENU_OPTION_PARENT, "Force Unlock"), "class").contains("disabled");
        boolean flag3 = sourceNavigateAngularLockReportPage().getElementsAttribute(format(SourceNavigateAngularLockReportPageElements.CONTEXT_MENU_OPTION_PARENT, "Transfer lock"), "class").contains("disabled");
        assertThat((!flag1) & flag2 & flag3)
    ...
    public void assertThatLockedReportUrl() {
        System.out.println("lock report url is"+sourceNavigateAngularLockReportPage().getUrl());
    ...
    public void assertThatContextMenuOptionIsEnabled(String option, boolean value) {
        assertThat(sourceNavigateAngularLockReportPage().verifyContextMenuOptionEnabled(format(SourceNavigateAngularLockReportPageElements.CONTEXT_MENU_OPTION_PARENT, option)))
                .isEqualTo(value);
    }
    ...
    public void assertThatDifficultyLevelWindowElements() {
        ...
        boolean diffLevelSubmitBtn = sourceNavigateAngularPopUpPage().doesElementExist(format(SourceNavigateAngularPopUpPageElements.ANY_BUTTON, "Submit"));
        boolean diffLevelCancelBtn = sourceNavigateAngularPopUpPage().doesElementExist(format(SourceNavigateAngularPopUpPageElements.ANY_BUTTON, "Cancel"));
    ...
    public void assertThatSectionDifficultyLevelValuesAsExpected(String expDiffVal, int startIndex) {
        ...
        if (sectionCount >= startIndex) {
            for (int index = startIndex; index < sectionCount - 1; index++) {
                sourceNavigateAngularPage().scrollToElement(format(SECTION_ROW, index));
                sourceNavigateAngularPage().scrollPageHorizontallyShiftEnd(format(SECTION_ROW, Integer.toString(index)));
                sourceNavigateAngularPage().scrollToTop();
                assertThat(sourceNavigateAngularPage().getElementsText(format(SECTION_ROW, index) + format(SourceNavigateAngularPageElements.COLUMN_ID_PATTERN, "difficultyLevel")))
    ...
    public void assertThatDeltaDifficultyLevelValuesAsExpected(String expDiffVal, int startIndex) {
        ...
        if (deltaCount >= startIndex) {
            for (int index = startIndex; index < deltaCount - 1; index++) {
                assertThat(sourceNavigateAngularPage().getElementsText(format(DELTA_ROW, index) + format(SourceNavigateAngularPageElements.COLUMN_ID_PATTERN, "difficultyLevel")))
    ...
    public void assertThatContentSetFiltered(String contentSet) {
        assertThat(sourceNavigateAngularLockReportPage().getElementsText(format(FIRST_CONTENT_SET, contentSet)).equals(contentSet))
                .as("The contentset is filtered to " + contentSet)
                .isTrue();
    }

    public void assertThatRenditionStatusFiltered(String renditionStatus) {
        assertThat(sourceNavigateAngularLockReportPage().getElementsText(format(FIRST_RENDITION_STATUS, renditionStatus)).equals(renditionStatus))
    ...
    public void assertThatDeltaPropertiesTabColumnValuesIsEnabled(String values, boolean value) {
        if (value) {
            assertThat(sourceNavigateAngularPage().isElementEnabled(format(SourceNavigateAngularDeltaPageElements.DELTA_PROPERTIES_VIEWMODE_ELEMENTS, values)))
                    .as("Delta Properties Column values are disabled")
                    .isTrue();

        } else {
            assertThat(sourceNavigateAngularPage().isElementDisabled(format(SourceNavigateAngularDeltaPageElements.DELTA_PROPERTIES_DROPDOWN_ELEMENTS, values))
            ).as("Delta Properties column values are enabled")
                    .isFalse();
        }
    }

    public void assertThatDeltaPropertiesValuesAsExpected(String actual, String expDiffVal, int startIndex) {
        assertThat(sourceNavigateAngularPage().getElementsText(format(DELTA_ROW, startIndex) + format(COLUMN_ID_PATTERN, actual)))
