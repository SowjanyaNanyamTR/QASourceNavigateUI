...
public class SNGeneralLineagePageTests extends SourceNavigateAngularAssertions {
    ...
    public void mockData(TestInfo testInfo) {
        if (!(testInfo.getDisplayName().equals("verifyLineageTabClearFilterTest()"))) {
    ...
    @LEGAL
    @LOG
    public void verifyLineageTabClearFilterTest()
    {
        sourceNavigateAngularPage().click(FIRST_RENDITION_ROW);
        sourceNavigateAngularPage().click(LINEAGE_TAB);
        String totalNumberOfLineages = sourceNavigateAngularLineagePage().getTotalNumberOfLineage();
        sourceNavigateAngularFiltersAndSortsPage().openMenuColumnHeaderForTabs(COLUMN_FILTER_BUTTON_PATTERN_LINEAGE_TAB, CONTENT_TYPE);
        ...
        assertThatFilteredRenditionsNumberLessThanTotalRenditions(sourceNavigateAngularLineagePage().getTotalNumberOfLineage(), totalNumberOfLineages);

        //Clearing Filter
        sourceNavigateAngularLineagePage().isElementDisplayed(CLEAR_FILTERS_LINEAGE_TAB);
        sourceNavigateAngularLineagePage().clickLineageTabClearFiltersButton();
        DateAndTimeUtils.takeNap(DateAndTimeUtils.TWO_SECONDS);

        //verify current section numbers is equal to the total section numbers
        assertThatTotalRenditionsNumberMatchingToExpected(sourceNavigateAngularLineagePage().getTotalNumberOfLineage(), totalNumberOfLineages);
